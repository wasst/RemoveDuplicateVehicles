...
FROM         dbo.vLink_HRHMN_EMPL_REF HREmpRef INNER JOIN
                      dbo.vLink_HRCOM_ADDRESS_REF HRAddress ON HREmpRef.ADDRSS_STREET_CODE = HRAddress.ADDRESS_CODE INNER JOIN
                      dbo.vLink_HRHMN_EMPLJOB_DTL HRJobDtl ON HREmpRef.ID_NUMBER = HRJobDtl.ID_NUMBER INNER JOIN
                      dbo.vLink_HRSTC_CODE Division ON HRJobDtl.DEPARTMENT_CODE = Division.CODE_ID INNER JOIN
                      dbo.vLink_HRSTC_CODE Position ON HRJobDtl.JOB_TITLE_CODE = Position.CODE_ID LEFT OUTER JOIN
                      dbo.vLink_HRSTC_CODE Unit ON HRJobDtl.WORK_GROUP = Unit.CODE_ID
WHERE     (Division.CODE_TYPE = 'DEPARTMENT_CODE') AND (HRJobDtl.EFFECTIVE_DATE =
                          (SELECT     MAX(effective_date)
                            FROM          vLink_HRHMN_EMPLJOB_DTL t1
